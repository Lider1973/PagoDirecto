...
Module ServiSwitch_Initialization
    ...
    Sub Main(ByVal args() As String)
        ...
        Show_Message_Console("           Iniciando sistema, Modulo ROUTERS                  ", COLOR_BLACK, COLOR_GREEN, 2, TRACE_LOW, 0)
        ...
        If Validate_Queues() <> SUCCESSFUL Then
            GoTo MainExit
        End If
        ...
        Start_Process_Look_Iteract_OBJDC()
...
MainExit:
    ...
    Private Function Validate_Queues() As Byte
        Dim ErrorCode As Byte = PROCESS_ERROR


        GetPathQueueName(MyName, 0, Router_Request_Queue_Name)
        If Verify_Queue_Resource(Router_Request_Queue_Name) <> SUCCESSFUL Then
            Return ErrorCode
        End If

        GetPathQueueName(MyName, 1, Router_Reply_Queue_Name)
        If Verify_Queue_Resource(Router_Reply_Queue_Name) <> SUCCESSFUL Then
            Return ErrorCode
        End If

        GetPathQueueName(MyName, 4, Router_Command_Queue_Name)
        If Verify_Queue_Resource(Router_Command_Queue_Name) <> SUCCESSFUL Then
            Return ErrorCode
        End If

        GetPathQueueName(MyName, 3, SAF_Router_Queue_Name)
        If Verify_Queue_Resource(SAF_Router_Queue_Name) <> SUCCESSFUL Then
            Return ErrorCode
        End If

        EnableTime = My.Settings.EnableTime
