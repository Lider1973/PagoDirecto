...
Public Class ServiSwitch_ProcessCommand
    ...
    Dim EnvironmentData() As String = {"HOMEPATH", "COMPUTERNAME", "USERDNSDOMAIN", "USERDOMAIN", "NUMBER_OF_PROCESSORS", "USERNAME", "PROCESSOR_ARCHITECTURE"}
    ...
    Private Sub Process_Send_Status_Commander()
        ...
        Do While True
            ...
            Put_Message_To_Manager(CommandLine, g_CommanderQueue, 0)
            ClearMemory()
            Thread.Sleep(5000)
        Loop
    End Sub
...
Exiting_Thread:
    ...
    Private Sub Evaluate_Status_Received(ByVal CommandLine As String)
        ...
        Dim ImgIdx As Byte = 2
        CommandLine += GetDateTime() & "|" & ImgIdx & "|" & ValMem
        Put_Message_To_Manager(CommandLine, g_CommanderQueue, 3)
    ...
    Private Sub Get_Current_Data_Process(ByVal CommandLine As String)
        Dim ProcessData As String = String.Empty
        Dim parms() As String = CommandLine.Split("|")

        Try
            Dim CurrentProcess As Process = Process.GetCurrentProcess()
            ProcessData = "Process Name     :" & CurrentProcess.ProcessName & ControlChars.NewLine
            ProcessData += "Id               :" & CurrentProcess.Id.ToString & ControlChars.NewLine
            ProcessData += "Window Name      :" & CurrentProcess.MainWindowTitle & ControlChars.NewLine
            ProcessData += "Start Time       :" & CurrentProcess.StartTime & ControlChars.NewLine
            ProcessData += "Threads          :" & CurrentProcess.Threads.Count & ControlChars.NewLine
            ProcessData += "Processor time   :" & (CurrentProcess.WorkingSet64 / 1024) & ControlChars.NewLine
            For Each de As DictionaryEntry In Environment.GetEnvironmentVariables()
                If EnvironmentData.Contains(de.Key) Then
                    ProcessData = ProcessData & de.Key.ToString.PadRight(17, " ") & ":" & de.Value & ControlChars.NewLine
                End If
            Next
        Catch ex As Exception
            Show_Message_Console("Exception getting process info:" & ex.Message, COLOR_BLACK, COLOR_RED, 0, TRACE_LOW, 1)
        End Try
        ProcessData = "REP|" & parms(1) & "|" & parms(2) & "|" & ProcessData
        Put_Message_To_Manager(ProcessData, g_CommanderQueue, 4)
    End Sub
